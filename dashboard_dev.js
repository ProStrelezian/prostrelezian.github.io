...
class ZlanDashboard {
    constructor(encodedUrl, enableTwitchLive = false, avatarUrl = null, is2026Format = false, useMockData = false) {
        console.log(`🛠️ [DEV] ZlanDashboard initialisé (Format 2026 : ${is2026Format} | Mock : ${useMockData})`);
        ...
        if (this.enableTwitchLive) {
            console.log("🎥 [DEV] Surveillance Twitch activée");
    ...
    async init() {
        ...
        // --- MODE MOCK ---
        if (this.useMockData) {
            console.log("🤡 [DEV] Mode MOCK activé : injection des fausses données hors ligne.");
            this.buildDashboard(this.getMockData());
            this.isFetching = false;
            
            const timeString = new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
            if (statusEl) statusEl.innerHTML = `<span class="w-2.5 h-2.5 bg-[var(--pixel-violet)] shadow-[0_0_8px_rgba(145,70,255,1)]"></span> <span style="color: var(--pixel-violet)" class="whitespace-nowrap">MOCK OK (${timeString})</span>`;
            
            setTimeout(() => this.init(), 30000); // Simuler un rafraîchissement
            return;
        }

        try {
            console.log(`🌐 [DEV] Récupération des données depuis Google Sheets...`);
            ...
            Papa.parse(csvData, {
                ...
                complete: (results) => {
                    console.log(`📊 [DEV] Parsing complet, ${results?.data?.length || 0} lignes récupérées.`);
                    if (results?.data) this.buildDashboard(results.data);
                    this.isFetching = false;
                    setTimeout(() => this.init(), 30000);
                },
                error: (err) => {
                    console.error("❌ [DEV] Erreur Parse:", err);
                    const timeString = new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
            ...
            console.error("❌ [DEV] Erreur Fetch :", error);
            const timeString = new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
    ...
    buildDashboard(data) {
        ...
        console.log("🏗️ [DEV] Construction du HTML du tableau de bord...");
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            // --- 3. PHASE DE SEEDING ---
                ...
                let seedingScore = "";
                let j = i + 1;
                while (j < data.length && !has(String(data[j][0] || ""), "PHASE DE KNOCKOUT") && j !== lastValidRowIndex) {
                    let subRow = data[j];
        ...
        if (container.innerHTML !== fullHtml && fullHtml.trim() !== "") {
            ...
            console.log("✅ [DEV] DOM mis à jour avec succès.");
        }
    ...
    async checkTwitchLive() {
        ...
        await Promise.all(streamers.map(async (streamer) => {
            try {
                ...
                console.error(`❌ [DEV] Erreur Twitch ${streamer}:`, e);
            }
        }));
    }
